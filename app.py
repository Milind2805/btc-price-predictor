...
with col_f:
    st.metric("Highest Price", 
              f"${float(filtered_df['Close'].max()):,.2f}")
with col_g:
    st.metric("Lowest Price", 
              f"${float(filtered_df['Close'].min()):,.2f}")
with col_h:
    st.metric("Average Price", 
              f"${float(filtered_df['Close'].mean()):,.2f}")
with col_i:
    total_return = ((float(filtered_df['Close'].iloc[-1]) / 
                     float(filtered_df['Close'].iloc[0])) - 1) * 100
